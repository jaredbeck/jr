...
public class JRConnectionManager implements TuioListener {
	...
	public void buildTree ( ) {
		...
		/* Acquire an exclusive lock on the tree before modifying it -Jared 2/11/10 */
		synchronized(jrTree) {
	...
	public void updateTuioObject(TuioObject tobj) {
		...
		if ( n != null ) { 
			try {
				...
				/* Update node position.  This may cause tree to change */
				n.setPosition( jrto.getX(), jrto.getY() );
				
				/* Now we might want to buildTree() to reflect the new position, but
				buildTree() locks the tree, causing the synth to skip .. -Jared 2/11/10 */
			}
