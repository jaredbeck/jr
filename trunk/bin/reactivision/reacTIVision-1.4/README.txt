...
Control messages via UDP port 3333 to any listening client application. 
It defines and implements the TUIO protocol, which was
specially designed for transmitting the state of tangible objects
and multi-touch events on a table surface. 
As an alternative reacTIVision is also able to send MIDI messages.

The framework includes a set of example client projects
for various programming languages, which serve as a base for
the development of tangible user interface or multi-touch
applications. Example projects are available for languages such as:
C/C++, Java, C#, Processing, Pure Data, Max/MSP, Flash, 
SuperCollider, Python, Smalltalk and Quartz Composer. 
...
best supported as well as a few V4L and V4L2 USB cameras.


Fiducial Symbols
----------------
This application was designed to track specially designed fiducial
markers. You will find the default "amoeba" set of 180 fiducials within
...
obviously not work at all without a camera.

Under Windows and MacOS X the application will ask to select and 
configure the camera upon startup. Once configured the application 
will show a video window with the current camera image in B&W.
...
light conditions with bad finger contrast for example.
You can gradually lower the value just until noise appears in the image.
...
some cases can simplify the recognition performance of both the finger
...
distortion algorithm only corrects the found positions instead 
of the full image.
